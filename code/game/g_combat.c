...
void TossClientItems( gentity_t *self ) {
	gitem_t		*item;
	//int			weapon; //qlone - instaGib
	float		angle;
	int			i;
	gentity_t	*drop;

	//qlone - instaGib or no items on map
	// we don't drop any weapons in instaGib
	if (!g_removeAllItems.integer && !g_instaGib.integer) {
		int weapon;
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
	...
	//qlone - self damage
	if (targ == attacker && g_noSelfDamage.integer == 2) {
		asave = 0;
	}
	else {
	//qlone - self damage
