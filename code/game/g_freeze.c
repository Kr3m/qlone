...
qboolean Set_spectator( gentity_t *ent ) {
	...
	if ( ent->r.svFlags & SVF_BOT ) {
		...
		ent->client->sess.spectatorNum = 0;
		//qlone
...
void player_freeze( gentity_t *self, gentity_t *attacker, int mod ) {
	//qlone
	// Do nothing unless it's enabled server-side
...
void team_wins( int team ) {
	...
	for ( i = 0; i < g_maxclients.integer; i++ ) {
		...
		ammo = trap_Cvar_VariableIntegerValue("g_startAmmoMG");
		cl->ps.ammo[ WP_MACHINEGUN ] = ammo;
		//qlone - custom ammo
...
void SpawnWeapon( gclient_t *client ) {
	...
	if ( g_wpflags.integer & 2 ) {
		...
		ammo = trap_Cvar_VariableIntegerValue("g_startAmmoSG");
		client->ps.ammo[ WP_SHOTGUN ] = ammo;
		//qlone - custom ammo
	}
	if ( g_wpflags.integer & 4 ) {
		...
		ammo = trap_Cvar_VariableIntegerValue("g_startAmmoGL");
		client->ps.ammo[ WP_GRENADE_LAUNCHER ] = ammo;
		//qlone - custom ammo
	}
	if ( g_wpflags.integer & 8 ) {
		...
		ammo = trap_Cvar_VariableIntegerValue("g_startAmmoRL");
		client->ps.ammo[ WP_ROCKET_LAUNCHER ] = ammo;
		//qlone - custom ammo
	}
	if ( g_wpflags.integer & 16 ) {
		...
		ammo = trap_Cvar_VariableIntegerValue("g_startAmmoLG");
		client->ps.ammo[ WP_LIGHTNING ] = ammo;
		//qlone - custom ammo
	}
	if ( g_wpflags.integer & 32 ) {
		...
		ammo = trap_Cvar_VariableIntegerValue("g_startAmmoRG");
		client->ps.ammo[ WP_RAILGUN ] = ammo;
		//qlone - custom ammo
	}
	if ( g_wpflags.integer & 64 ) {
		...
		ammo = trap_Cvar_VariableIntegerValue("g_startAmmoPG");
		client->ps.ammo[ WP_PLASMAGUN ] = ammo;
		//qlone - custom ammo
	}
	if ( g_wpflags.integer & 128 ) {
		...
		ammo = trap_Cvar_VariableIntegerValue("g_startAmmoBFG");
		client->ps.ammo[ WP_BFG ] = ammo;
		//qlone - custom ammo
	}
#ifdef MISSIONPACK
	if ( g_wpflags.integer & 256 ) {
		...
		ammo = trap_Cvar_VariableIntegerValue("g_startAmmoNG");
		client->ps.ammo[ WP_NAILGUN ] = ammo;
		//qlone - custom ammo
	}
	if ( g_wpflags.integer & 512 ) {
		...
		ammo = trap_Cvar_VariableIntegerValue("g_startAmmoPL");
		client->ps.ammo[ WP_PROX_LAUNCHER ] = ammo;
		//qlone - custom ammo
	}
	if ( g_wpflags.integer & 1024 ) {
		...
		ammo = trap_Cvar_VariableIntegerValue("g_startAmmoCG");
		client->ps.ammo[ WP_CHAINGUN ] = ammo;
		//qlone - custom ammo
	}
#endif
