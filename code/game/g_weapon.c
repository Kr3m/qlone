	...
	*/
	...
	for (i = 0; i < 10; i++) {
		...
		//qlone - optional FreezeTag
			tent = G_TempEntity( tr.endpos, EV_BULLET_HIT_FLESH );
			tent->s.eventParm = traceEnt->s.number;
			//qlone - unlagged - EXTRA for freeze
			// the same "attack prediction #2" logic should apply here
			tent->s.clientNum = ent->s.clientNum;
			//qlone - unlagged - EXTRA for freeze
		//freeze
		} else {
...
//freeze
	...
	damage = g_instaGib.integer > 0 ? 500 * s_quadFactor : 100 * s_quadFactor;
	//qlone - instaGib
