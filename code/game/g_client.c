...
//		client->areabits = G_Alloc( (trap_AAS_PointReachabilityAreaIndex( NULL ) + 7) / 8 );
	...
	//qlone - optional FreezeTag
	// announce if the FreezeTag variant is enabled
	if ( g_freezeTag.integer ) {
		trap_SendServerCommand( clientNum, "print \"This game has FreezeTag variant enabled\n\"" );
	}
	//qlone - optional FreezeTag
...
//	client->areabits = savedAreaBits;
	...
	//qlone - instaGib
	// only the railgun will be available
	...
	if (!level.intermissiontime) {
		...
		//freeze
			...
			//qlone - was only for FreezeTag, made it avail for all
